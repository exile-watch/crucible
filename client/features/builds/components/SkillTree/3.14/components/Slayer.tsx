import * as React from 'react';

function SvgSlayer(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-490.795 8229.329 1450 1450" {...props}>
      <g
        style={{
          transform: 'translate(calc(50% - 475px),calc(50% - 320px))',
        }}
      >
        <path
          d="M441.471 8474.383a493 493 0 00-149.488-175.028"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-10143-34215"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-10143-45696"
          d="M441.471 8474.383L292.324 8530.7"
        />
        <path
          d="M-150.14 8229.33a493 493 0 00-137.433 70.025"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-15286-3184"
        />
        <path
          d="M-287.573 8299.355a493 493 0 00-149.488 175.028"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-15286-62817"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-20954-38180"
          d="M337.205 8698.2h158"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-20954-33795"
          d="M337.205 8698.2h-335"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-33795-42293"
          d="M2.205 8698.2l-290.119-167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-33795-45696"
          d="M2.205 8698.2l290.119-167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-33795-61393"
          d="M2.205 8698.2h-335"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-33795-50845"
          d="M2.205 8698.2l290.119 167.5"
        />
        <path
          d="M291.983 8299.355a493 493 0 00-137.433-70.026"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-34215-34484"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-42293-62817"
          d="M-287.914 8530.7l-149.147-56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-50845-16306"
          d="M292.324 8865.7l149.147 56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-61393-17315"
          d="M-332.795 8698.2h-158"
        />
        <circle
          cx={-150.14}
          cy={8229.329}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-3184"
          className="out-15286 s0"
        />
        <circle
          cx={441.471}
          cy={8474.383}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-10143"
          className="out-34215-45696 s0"
        />
        <circle
          cx={-287.573}
          cy={8299.355}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-15286"
          className="out-3184-62817 s0"
        />
        <circle
          cx={441.471}
          cy={8922.017}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-16306"
          className="out-50845 s0"
        />
        <circle
          cx={-490.795}
          cy={8698.2}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-17315"
          className="out-61393 s0"
        />
        <circle
          cx={337.205}
          cy={8698.2}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-20954"
          className="out-38180-33795 s0"
        />
        <circle
          cx={2.205}
          cy={8698.2}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-33795"
          className="out-42293-45696-20954-61393-50845-50986 s0 root"
        />
        <circle
          cx={291.983}
          cy={8299.355}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-34215"
          className="out-34484-10143 s0"
        />
        <circle
          cx={154.55}
          cy={8229.329}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-34484"
          className="out-34215 s0"
        />
        <circle
          cx={495.205}
          cy={8698.2}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-38180"
          className="out-20954 s0"
        />
        <circle
          cx={-287.914}
          cy={8530.7}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-42293"
          className="out-62817-33795 s0"
        />
        <circle
          cx={292.324}
          cy={8530.7}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-45696"
          className="out-10143-33795 s0"
        />
        <circle
          cx={292.324}
          cy={8865.7}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-50845"
          className="out-16306-33795 s0"
        />
        <circle
          cx={-332.795}
          cy={8698.2}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-61393"
          className="out-17315-33795 s0"
        />
        <circle
          cx={-437.061}
          cy={8474.383}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-62817"
          className="out-15286-42293 s0"
        />
      </g>
    </svg>
  );
}

const MemoSvgSlayer = React.memo(SvgSlayer);
export default MemoSvgSlayer;
