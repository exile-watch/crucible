import * as React from 'react';

function SvgChieftain(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-8758.7 2368.114 1450 1450" {...props}>
      <g
        style={{
          transform: 'translate(calc(50% - 400px),calc(50% - 460px))',
        }}
      >
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-982-5029"
          d="M-8251.7 3097.499l56.317 149.147"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-982-24704"
          d="M-8251.7 3097.499l-167.5-290.119"
        />
        <path
          d="M-8677.7 3265.956a162 162 0 00162 0"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-1731-14996"
        />
        <path
          d="M-8758.7 3125.66a162 162 0 0081 140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-1731-42659"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-5643-31667"
          d="M-8129.081 2974.88l149.147 56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-5643-24704"
          d="M-8129.081 2974.88l-290.119-167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-6028-53095"
          d="M-8129.081 2639.88l149.147-56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-6028-24704"
          d="M-8129.081 2639.88l-290.119 167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9190-48480"
          d="M-8084.2 2807.38h158"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9190-24704"
          d="M-8084.2 2807.38h-335"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9971-50692"
          d="M-8559.496 2888.38l-118.204 96.984"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9971-24704"
          d="M-8559.496 2888.38l140.296-81"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-10238-61355"
          d="M-8419.2 2969.38l-15.5 156.28"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-10238-24704"
          d="M-8419.2 2969.38v-162"
        />
        <path
          d="M-8515.7 3265.956a162 162 0 0081-140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-14996-61355"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-24704-47486"
          d="M-8419.2 2807.38l167.5-290.119"
        />
        <path
          d="M-8677.7 2985.364a162 162 0 00-81 140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-42659-50692"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-47486-32249"
          d="M-8251.7 2517.261l56.317-149.147"
        />
        <circle
          cx={-8251.7}
          cy={3097.499}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-982"
          className="out-5029-24704 s0"
        />
        <circle
          cx={-8677.7}
          cy={3265.956}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-1731"
          className="out-14996-42659 s0"
        />
        <circle
          cx={-8195.383}
          cy={3246.646}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-5029"
          className="out-982 s0"
        />
        <circle
          cx={-8129.081}
          cy={2974.88}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-5643"
          className="out-31667-24704 s0"
        />
        <circle
          cx={-8129.081}
          cy={2639.88}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-6028"
          className="out-53095-24704 s0"
        />
        <circle
          cx={-8084.2}
          cy={2807.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-9190"
          className="out-48480-24704 s0"
        />
        <circle
          cx={-8559.496}
          cy={2888.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-9971"
          className="out-50692-24704 s0"
        />
        <circle
          cx={-8419.2}
          cy={2969.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-10238"
          className="out-61355-24704 s0"
        />
        <circle
          cx={-8515.7}
          cy={3265.956}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-14996"
          className="out-1731-61355 s0"
        />
        <circle
          cx={-8419.2}
          cy={2807.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-24704"
          className="out-982-9971-9190-10238-5643-6028-47486-47175 s0 root"
        />
        <circle
          cx={-7979.934}
          cy={3031.197}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-31667"
          className="out-5643 s0"
        />
        <circle
          cx={-8195.383}
          cy={2368.114}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-32249"
          className="out-47486 s0"
        />
        <circle
          cx={-8758.7}
          cy={3125.66}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-42659"
          className="out-1731-50692 s0"
        />
        <circle
          cx={-8251.7}
          cy={2517.261}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-47486"
          className="out-32249-24704 s0"
        />
        <circle
          cx={-7926.2}
          cy={2807.38}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-48480"
          className="out-9190 s0"
        />
        <circle
          cx={-8677.7}
          cy={2985.364}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-50692"
          className="out-42659-9971 s0"
        />
        <circle
          cx={-7979.934}
          cy={2583.563}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-53095"
          className="out-6028 s0"
        />
        <circle
          cx={-8434.7}
          cy={3125.66}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-61355"
          className="out-14996-10238 s0"
        />
      </g>
    </svg>
  );
}

const MemoSvgChieftain = React.memo(SvgChieftain);
export default MemoSvgChieftain;
