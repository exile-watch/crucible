import * as React from 'react';

function SvgChieftain(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="-8758.7 3818.114 1450 1450" {...props}>
      <g
        style={{
          transform: 'translate(calc(50% - 400px),calc(50% - 460px))',
        }}
      >
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-982-5029"
          className="in-5029-24704"
          d="M-8251.7 4547.499l56.317 149.147"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-982-24704"
          className="in-5029-24704"
          d="M-8251.7 4547.499l-167.5-290.119"
        />
        <path
          d="M-8677.7 4715.956a162 162 0 00162 0"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-1731-14996"
          className="in-14996-42659"
        />
        <path
          d="M-8758.7 4575.66a162 162 0 0081 140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-1731-42659"
          className="in-14996-42659"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-5643-31667"
          className="in-31667-24704"
          d="M-8129.081 4424.88l149.147 56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-5643-24704"
          className="in-31667-24704"
          d="M-8129.081 4424.88l-290.119-167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-6028-53095"
          className="in-53095-24704"
          d="M-8129.081 4089.88l149.147-56.317"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-6028-24704"
          className="in-53095-24704"
          d="M-8129.081 4089.88l-290.119 167.5"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9190-48480"
          className="in-48480-24704"
          d="M-8084.2 4257.38h158"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9190-24704"
          className="in-48480-24704"
          d="M-8084.2 4257.38h-335"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9971-50692"
          className="in-50692-24704"
          d="M-8559.496 4338.38l-118.204 96.984"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-9971-24704"
          className="in-50692-24704"
          d="M-8559.496 4338.38l140.296-81"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-10238-61355"
          className="in-61355-24704"
          d="M-8419.2 4419.38l-15.5 156.28"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-10238-24704"
          className="in-61355-24704"
          d="M-8419.2 4419.38v-162"
        />
        <path
          d="M-8515.7 4715.956a162 162 0 0081-140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-14996-61355"
          className="in-1731-61355"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-24704-47486"
          className="in-982-5643-6028-9190-9971-10238-47486-47175"
          d="M-8419.2 4257.38l167.5-290.119"
        />
        <path
          d="M-8677.7 4435.364a162 162 0 00-81 140.296"
          stroke="var(--secondary-bg)"
          fill="transparent"
          strokeWidth={30}
          id="skill-42659-50692"
          className="in-1731-50692"
        />
        <path
          stroke="var(--secondary-bg)"
          strokeWidth={30}
          id="skill-47486-32249"
          className="in-24704-32249"
          d="M-8251.7 3967.261l56.317-149.147"
        />
        <circle
          cx={-8251.7}
          cy={4547.499}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-982"
          className="out-5029-24704 s0 in-5029-24704"
        />
        <circle
          cx={-8677.7}
          cy={4715.956}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-1731"
          className="out-14996-42659 s0 in-14996-42659"
        />
        <circle
          cx={-8195.383}
          cy={4696.646}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-5029"
          className="out-982 s0 in-982"
        />
        <circle
          cx={-8129.081}
          cy={4424.88}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-5643"
          className="out-31667-24704 s0 in-31667-24704"
        />
        <circle
          cx={-8129.081}
          cy={4089.88}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-6028"
          className="out-53095-24704 s0 in-53095-24704"
        />
        <circle
          cx={-8084.2}
          cy={4257.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-9190"
          className="out-48480-24704 s0 in-48480-24704"
        />
        <circle
          cx={-8559.496}
          cy={4338.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-9971"
          className="out-50692-24704 s0 in-50692-24704"
        />
        <circle
          cx={-8419.2}
          cy={4419.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-10238"
          className="out-61355-24704 s0 in-61355-24704"
        />
        <circle
          cx={-8515.7}
          cy={4715.956}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-14996"
          className="out-1731-61355 s0 in-1731-61355"
        />
        <circle
          cx={-8419.2}
          cy={4257.38}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-24704"
          className="out-982-9971-9190-10238-5643-6028-47486-47175 s0 in-982-5643-6028-9190-9971-10238-47486-47175 root"
        />
        <circle
          cx={-7979.934}
          cy={4481.197}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-31667"
          className="out-5643 s0 in-5643"
        />
        <circle
          cx={-8195.383}
          cy={3818.114}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-32249"
          className="out-47486 s0 in-47486"
        />
        <circle
          cx={-8758.7}
          cy={4575.66}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-42659"
          className="out-1731-50692 s0 in-1731-50692"
        />
        <circle
          cx={-8251.7}
          cy={3967.261}
          r={30}
          fill="var(--secondary-bg)"
          id="skill-47486"
          className="out-32249-24704 s0 in-24704-32249"
        />
        <circle
          cx={-7926.2}
          cy={4257.38}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-48480"
          className="out-9190 s0 in-9190"
        />
        <circle
          cx={-8677.7}
          cy={4435.364}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-50692"
          className="out-42659-9971 s0 in-9971-42659"
        />
        <circle
          cx={-7979.934}
          cy={4033.563}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-53095"
          className="out-6028 s0 in-6028"
        />
        <circle
          cx={-8434.7}
          cy={4575.66}
          r={50}
          fill="var(--secondary-bg)"
          id="skill-61355"
          className="out-14996-10238 s0 in-10238-14996"
        />
      </g>
    </svg>
  );
}

const MemoSvgChieftain = React.memo(SvgChieftain);
export default MemoSvgChieftain;
